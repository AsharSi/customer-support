    ...
    <div className="flex-grow overflow-y-auto p-4 bg-gray-50 space-y-4">
      {messages.map((msg, index) => (
        <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
          <div className={`inline-block p-3 rounded-lg max-w-[70%] shadow-sm ${
...
const ResolveForm = ({ onSubmit, onCancel }) => {
  ...
  const ChatHeader = React.memo(({ chat, onResolve, onViewSocietyDetails, showSocietyDetails }) => (
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchAgentChats, 5000); // Fetch every 5 seconds
  ...
  useEffect(() => {
    if (selectedChat) {
      socket.emit('join', { thread_id: selectedChat.thread_id });
    }
    return () => {
      if (selectedChat) {
